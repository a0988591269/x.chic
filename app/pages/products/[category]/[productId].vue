  ...
  <div class="max-w-6xl mx-auto px-4 py-10">
    <div class="flex flex-col md:flex-row gap-10">
      ...
      <div class="md:w-1/2 flex flex-col items-center">
        <!-- 輪播主圖 -->
        <div class="w-full aspect-square bg-gray-100 rounded-xl flex items-center justify-center overflow-hidden relative">
          <img
      ...
      <div class="md:w-1/2 flex flex-col gap-4">
        ...
        <div class="flex items-center gap-3 mb-2">
          <span v-if="product.discountPrice < product.price" class="text-gray-700 font-bold text-2xl">
            ${{ product.discountPrice }}
          </span>
          <span v-if="product.discountPrice < product.price" class="text-gray-400 line-through text-lg">
            ${{ product.price }}
          </span>
          <span v-else class="text-gray-800 font-bold text-2xl">${{ product.price }}</span>
          <span v-if="product.discountPrice < product.price" class="ml-2 bg-gray-100 text-gray-500 text-sm font-semibold px-2 py-0.5 rounded">
            -{{ discountPercent }}%
          </span>
        </div>
        <div class="flex items-center gap-4 text-gray-600 text-base">
          ...
          <span class="flex items-center gap-1">
            <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="M10 15l-5.878 3.09 1.122-6.545L.488 6.91l6.561-.955L10 0l2.951 5.955 6.561.955-4.756 4.635 1.122 6.545z"/></svg>
            {{ product.rating }} / 5
          </span>
        </div>
        <div class="flex items-center gap-3 mt-2">
          <label for="qty" class="text-gray-700">購買數量</label>
          <select id="qty" v-model="quantity" class="border border-gray-300 rounded px-3 py-1 focus:outline-none focus:border-gray-500">
    ...
    <div class="mt-12">
      <div class="flex border-b border-gray-200 mb-6">
        ...
        >
          {{ tab.label }}
          <span v-if="tab.key === 'reviews'" class="ml-1 text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded">{{ product.reviews.length }}</span>
        </button>
      </div>
      <div>
        ...
        <div v-else-if="activeTab === 'reviews'">
          <div v-if="product.reviews.length === 0" class="text-gray-400">尚無評價</div>
    ...
    <div class="mt-14 grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div v-for="item in hotProducts" :key="item.id" class="bg-white rounded-lg shadow p-3 flex flex-col items-center">
      ...
      <div>
        ...
        <ol class="space-y-3">
          <li v-for="(item, idx) in topSales" :key="item.id" class="flex items-center gap-3">
            <span class="w-6 h-6 flex items-center justify-center bg-gray-200 text-gray-700 font-bold rounded-full">{{ idx + 1 }}</span>
